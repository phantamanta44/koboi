    ...
    if (it == "PREFIX") {
        ...
        TraceOpcode(parts[0], parts[1].split(',', '+').map { arg ->
            if (arg.startsWith('(')) {
                ...
                when (actualArg) {
                    "a8" -> ArgumentImmediateHighPointer()
                    "a16" -> ArgumentImmediatePointer()
                    "C" -> ArgumentRegisterHighPointer(actualArg)
                    else -> ArgumentRegisterPointer(actualArg)
                }
            } else {
                when (arg) {
                    "C", "NC", "Z", "NZ" -> if (parts[0] == "CALL" || parts[0] == "RET" || parts[0] == "JP" || parts[0] == "JR") {
                        ...
                        ArgumentRegister8(arg)
                    }
                    "A", "F", "B", "D", "E", "H", "L" -> ArgumentRegister8(arg)
                    "AF", "BC", "DE", "HL", "SP", "PC" -> ArgumentRegister16(arg)
                    "d8" -> ArgumentImmediateU8()
                    "d16", "a16" -> ArgumentImmediateU16()
                    "r8" -> ArgumentImmediateI8()
...
open class TraceOpcode(val name: String, val arguments: List<IArgument>?) {
    ...
    open fun collectArgs(cpu: Cpu): List<IArgumentValue> {
        if (arguments == null) return listOf()
        val args = mutableListOf<IArgumentValue>()
        var pc = cpu.regPC.read().toUnsignedInt() + 1
        for (argument in arguments) pc = argument.read(cpu, pc, args)
        return args
    }
...
class TraceOpcodeCb : TraceOpcode("PREFIX CB", listOf()) {
    ...
    override fun collectArgs(cpu: Cpu): List<IArgumentValue> {
        val opcode = cpu.memory.read(cpu.regPC.read().toUnsignedInt() + 1).toUnsignedInt()
        return listOf(ArgValueCb(opcode)) + cbTraceOpcodes[opcode].collectArgs(cpu)
    }

    private inner class ArgValueCb(private val opcode: Int) : IArgumentValue {

        override fun stringify(): String = cbMnemonics[opcode]
...
val cbTraceOpcodes: List<TraceOpcode> = cbMnemonics.map {
    ...
    TraceOpcode(it.substring(0, regIndex), listOf(if (reg == "(HL)") {
        ArgumentRegisterPointer("HL")
    } else {
        ArgumentRegister8(reg)
    }))
}