...
class GbRenderer(ctrl: LcdControlRegister, display: IDisplay, cpu: Cpu, vram: MemoryBankSwitcher,
                 private val palBg: MonoPaletteRegister, private val palSprite0: MonoPaletteRegister, private val palSprite1: MonoPaletteRegister)
    : ScanLineRenderer(ctrl, display, cpu, vram) {
    ...
    override fun renderScanLine(y: Int) {
        ...
        if (ctrl.bgState) {
            ...
            val rowBgTiles = getTileRow(bgTileMapType, tileAbsY, 0).map { getTileData(it, ctrl.tileDataSelect, 0) }
            ...
            for (x in 0..159) {
                ...
                val col = palBg.getColour(colIndex)
                if (colIndex == 0) bgPriorities[x] = 0
                writePixel(x, col)
            }
        } else {
            bgPriorities.fill(0)
        ...
        if (ctrl.windowDisplayEnable) {
            ...
            if (y >= windowY) {
                ...
                val rowWinTiles = getTileRow(winTileMapType, winAbsY, 0).map { getTileData(it, ctrl.tileDataSelect, 0) }
                ...
                for (x in 0..159) {
                    if (x in windowXRange) {
                        ...
                        val index = winTileOffsetY * 2
                        val pixLower = tileData[index]
                        val pixUpper = tileData[index + 1]
                        val xShiftFactor = 7 - (winAbsX % 8)
                        val mask = 1 shl xShiftFactor
                        val colIndex = ((pixLower.toInt() and mask) ushr xShiftFactor) or
                                (((pixUpper.toInt() and mask) ushr xShiftFactor) shl 1)
                        val col = palBg.getColour(colIndex)
                ...
                if (y >= sY && y < sY + spriteHeight) {
                    ...
                    if (sX > -8 && sX < 160) {
                        ...
                        val tileData = if (doubleHeight) {
                            if (spriteOffsetY >= 8) {
                                spriteOffsetY %= 8
                                getTileData(tileIndex or 1, true, 0)
                            } else {
                                getTileData(tileIndex and 0b11111110.toByte(), true, 0)
                            }
                        } else {
                            getTileData(tileIndex, true, 0)
                        }
                        for (x in sX..(sX + 7)) {
                            if (x in 0..159) {
                                if (bgPriorities[x] == 0 || (meta and 0x80 == 0 && bgPriorities[x] == 1)) {
                                    ...
                                    val colIndex = ((pixLower.toInt() and mask) ushr xShiftFactor) or
                                            (((pixUpper.toInt() and mask) ushr xShiftFactor) shl 1)
                                    if (colIndex != 0) {
                                        val col = (if (meta and 0x08 == 0) palSprite0 else palSprite1).getColour(colIndex)
                                        writePixel(x, col)
                                    }
                                }
                            }
...
class GbcRenderer(ctrl: LcdControlRegister, display: IDisplay, cpu: Cpu, vram: MemoryBankSwitcher,
    ...
    override fun renderScanLine(y: Int) {
        // draw bg/window
        val setPriority = ctrl.bgState
        bgPriorities.fill(if (setPriority) 1 else 0)
        val tileAbsY = (cpu.memory.read(0xFF42).toUnsignedInt() + y) % 256
        val tileOffsetY = tileAbsY % 8
        val bgTileMapType = ctrl.bgTileMapDisplaySelect
        ...
        if (ctrl.windowDisplayEnable) {
            ...
            if (y >= windowY) {
                ...
                val rowWinTiles = getTileRow(winTileMapType, winAbsY, 0)
                        .zip(rowWinMeta) { tile, meta -> getTileData(tile, tileDataType, (meta.toInt() and 0x8) ushr 3) }
                val windowXRange = windowX..(windowX + 159)
                val winTileOffsetY = winAbsY % 8
                for (x in 0..159) {
                    if (x in windowXRange) {
                        ...
                        if (setPriority) {
                            if (colIndex == 0) {
                                bgPriorities[x] = 0
                            } else if (tileMeta and 0x80 != 0) {
                                bgPriorities[x] = 2
                            }
                        }
                        writePixel(x, col)
                    } else {
                        drawBgPixel(x, tileOffsetY, scrollX, rowBgMeta, rowBgTiles, setPriority)
                    }
                }
            } else {
                for (x in 0..159) drawBgPixel(x, tileOffsetY, scrollX, rowBgMeta, rowBgTiles, setPriority)
            }
        } else {
            for (x in 0..159) drawBgPixel(x, tileOffsetY, scrollX, rowBgMeta, rowBgTiles, setPriority)
        }
    ...
    private fun drawBgPixel(x: Int, tileOffsetY: Int, scrollX: Int, rowBgMeta: IMemoryRange, rowBgTiles: List<IMemoryRange>, setPriority: Boolean) {
        val tileAbsX = (scrollX + x) % 256
        val rowIndex = floor(tileAbsX / 8.0).toInt()
